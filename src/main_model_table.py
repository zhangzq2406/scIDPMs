...
from src.scIDPMs import diff_CSDI
...
class CSDI_base(nn.Module):
    ...
    def impute_new(self, observed_data, cond_mask, side_info, n_samples, target_mask):
        B, K, L = observed_data.shape
        imputed_samples = torch.zeros(B, n_samples, K, L).to(self.device)
        ...
        for i in range(n_samples):
            ...
            for t in range(self.num_steps - 1, -1, -1):
                ...
                current_sample = (coeff1 * (current_sample - coeff2 * predicted)) * target_mask

                if t > 0:
                    noise = torch.randn_like(current_sample)
                    sigma = ((1.0 - self.alpha[t - 1]) / (1.0 - self.alpha[t]) * self.beta[t]) ** 0.5
                    current_sample = (current_sample + sigma * noise) * target_mask
    ...
    def genera(self, batch, n_samples):
        ...
        with torch.no_grad():
            ...
            target_mask = true_masks
            side_info = self.get_side_info(observed_tp, cond_mask)
            samples = self.impute_new(observed_data, cond_mask, side_info, n_samples, target_mask)
        return samples, observed_data, target_mask, observed_mask, observed_tp


class scIDPMs(CSDI_base):
    def __init__(self, config, device, target_dim=1):
        super(scIDPMs, self).__init__(target_dim, config, device)
    ...
    def process_data(self, batch):
        ...
        true_masks = batch["true_masks"][:, np.newaxis, :]
        true_masks = true_masks.to(self.device).float()
